...
const BusinessModerationBanner = ({ item, darkMode = false, compact = false }) => {
  if (!item || !isBusinessModeratedBlocked(item)) return null;

  const status = getBusinessModerationStatus(item);
  const message = getBusinessModerationOwnerMessage(item);
  ...
  return (
    ...
    >
      <View style={styles.headerRow}>
        ...
        >
          {getBusinessModerationStatusLabel(item)}
        </Text>
      </View>
...
export default BusinessModerationBanner;
