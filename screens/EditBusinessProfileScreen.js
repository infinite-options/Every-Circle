...
const EditBusinessProfileScreen = ({ route, navigation }) => {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (response.status === 200) {
        ...
        // Save options one service at a time — parallel POSTs to the same bs_uid race on the backend
        // (soft-delete + re-insert) and return 500 even when the JSON payload is valid.
        const optionResults = [];
        ...
        for (const localSvc of servicesForPayload) {
          ...
          if (!uid) {
            uid =
              returnedServices.find(
                (r) => String(r.bs_service_name || "").trim() === String(localSvc.bs_service_name || "").trim() && String(r.bs_uid || "").trim() !== "",
              )?.bs_uid || null;
          }
        ...
        if (optionFailures.length > 0) {
          const names = optionFailures.map((f) => f.name).filter(Boolean).join(", ");
          throw new Error(
  ...
  const serviceUidsKey = useMemo(
    () => services.map((s) => String(s.bs_uid || "").trim()).filter(Boolean).join("|"),
    [services],
  );
  ...
  const mergePendingServiceEditIntoList = (formSource = serviceForm) => {
    if (editingServiceIndex === null) return null;
    const row = buildServiceRowForList(formSource);
    if (!row) return null;
    const index = editingServiceIndex;
    setServices((prev) => {
      const updated = [...prev];
      updated[index] = row;
      return updated;
  ...
  const handleEditService = (service, index) => {
    if (showServiceForm && editingServiceIndex !== null && editingServiceIndex !== index) {
      const resolvedPending =
        productTagInput.trim()
          ...
          : serviceForm;
      mergePendingServiceEditIntoList(resolvedPending);
    }
          ...
          <View style={{ flex: 1, gap: 8 }}>
            <View style={{ flexDirection: "row", gap: 8, flexWrap: "wrap" }}>
              <TouchableOpacity
                ...
                onPress={() => {
                  setServiceForm((prev) => ({ ...prev, bs_special_instructions_enabled: 1, _serviceOptionsTouched: true }));
                  setIsChanged(true);
                }}
              >
                <Text
              ...
              <View style={{ flexDirection: "row", alignItems: "center", gap: 8 }}>
                ...
                <TextInput
                  ...
                  onChangeText={(t) => {
                    setServiceForm((prev) => ({
                      ...prev,
                      bs_special_instructions_max_chars: t.replace(/\D/g, ""),
            ...
            <View key={service.bs_uid ? String(service.bs_uid) : `svc-${idx}`} style={styles.productCardEditWrapper}>
              ...
              <View style={styles.productCardRow}>
                <ProductCard
                  ...
                  showEditButton={true}
                  darkMode={darkMode}
                  choiceGroups={service.bs_choice_groups}
                />
                <TouchableOpacity
