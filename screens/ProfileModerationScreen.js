...
import {
  ...
  getProfileModerationOwnerMessage,
...
export default function ProfileModerationScreen() {
  ...
  const status = getProfileModerationStatus(moderationItem);
  ...
  const handleAcknowledge = async () => {
    ...
    try {
      ...
      const result = await acknowledgeProfileModeration({
        profilePersonalUid: profileUid,
        profileUid,
      });
      ...
      Alert.alert(
        ...
        already
          ? "This take-down was already acknowledged. Your profile remains hidden."
          : "You acknowledged this take-down. Your profile remains hidden from others.",
      );
    } catch (e) {
      console.error("ProfileModerationScreen - acknowledge failed:", e);
      Alert.alert("Error", e?.message || "Failed to acknowledge profile take-down.");
    } finally {
  ...
  return (
    <SafeAreaView style={[styles.safeArea, darkMode && styles.darkSafeArea]}>
      <AppHeader title='YOUR PROFILE' {...getHeaderColors("profile")} />
      ...
      <ScrollView contentContainerStyle={styles.page}>
        ...
        {profileName ? <Text style={[styles.profileName, darkMode && styles.darkSubtitle]}>{profileName}</Text> : null}
        ...
        <View style={[styles.card, darkMode && styles.darkCard]}>
          ...
          <TouchableOpacity style={styles.contactRow} onPress={openSupportEmail} activeOpacity={0.7}>
            ...
            <Text style={styles.contactLink}>{PROFILE_SUPPORT_EMAIL}</Text>
          </TouchableOpacity>
        </View>
        ...
        {isTakenDown ? (
          <View style={[styles.card, darkMode && styles.darkCard]}>
            <Text style={[styles.bodyText, darkMode && styles.darkBodyText]}>
              While your profile is taken down, it is hidden from search and other members cannot view it.
            </Text>
          </View>
        ) : null}
        ...
        {isPending ? (
          <View style={[styles.card, darkMode && styles.darkCard]}>
            <Text style={[styles.bodyText, darkMode && styles.darkBodyText]}>
              Your profile is temporarily hidden while our team reviews reports. You will regain access if your profile is approved.
            </Text>
          </View>
        ) : null}
